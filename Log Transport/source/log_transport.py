...
conn = pymysql.connect(host='localhost', user='root', passwd='toor', db='test', charset='utf8')       #db 연결(host='192.168.43.89', user='root', passwd='1234', db='mysql')
...
for second in range(0, 86400):
    ...
    if msg != None:     #테이블에 데이터가 있으면
        curs.execute("select start_proc from test order by start_proc desc limit 1")       #table name = proc_log, DB에 저장된 데이터 중 가장 최근 시작한 프로세스
        ...
        for i in range (0, len(process_name)):       #DB에 저장될 컨텐츠
            if int(start_proc[i]) < int(last_proc[0]):      #start_proc에 저장된 데이터 중 last_proc 보다 작을 경우, 다음 인덱스로 넘어감, start_proc + index
                ...
                sql = """insert into test(mac, proc_name, start_proc) values (%s, %s, %s)"""  #table name = proc_log
    ...
    if msg == None:     # 테이블에 데이터가 없을 경우
        for i in range (0, len(process_name)):       #DB에 저장될 컨텐츠
            sql1 = """insert into test(mac, proc_name, start_proc) values (%s, %s, %s)"""  #table name = proc_log
            curs.execute(sql1, (arrinfo[2], process_name[i], start_proc[i]))
            conn.commit()

    print(datetime.datetime.now())
    # print(int(last_proc[0]))
    # print("index : ", index)
    time.sleep(20)      #30초동안 지연
    second += 20
